...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 1) {
      ...
      // Subsidy is cut in half every 500000 blocks, which will occur approximately every year
      nSubsidy >>= (nHeight / 500000); // Standardcoin: 500k blocks in 1 year
    }

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 60; // Standardcoin: 1 minute
static const int64 nTargetSpacing = 60; // Standardcoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Standardcoin: This fixes an issue where a 51% attack can change difficulty at will.
    ...
    if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) { return bnProofOfWorkLimit.GetCompact(); }
        ...
        for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
                ...
                if (LatestBlockTime < BlockReading->GetBlockTime()) {
                  if (BlockReading->nHeight > 18000) LatestBlockTime = BlockReading->GetBlockTime();
                }
                PastRateActualSeconds = LatestBlockTime - BlockReading->GetBlockTime();

                PastRateTargetSeconds = TargetBlocksSpacingSeconds * PastBlocksMass;
                PastRateAdjustmentRatio = double(1);
                if (BlockReading->nHeight > 18000) {
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Standardcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xc0, 0xc0, 0xc0, 0xc0 }; // Standardcoin: increase each by adding 2 to bitcoin's value.
...
void static StandardcoinMiner(CWallet *pwallet)
{
    printf("StandardcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("standardcoin-miner");
    ...
    try { loop {
        ...
        printf("Running StandardcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("StandardcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StandardcoinMiner, pwallet));
}
